...
각 검출에 `origin` 을 붙이되 그건 잠정값이다 — `compare` 가 발췌를 원본과 문자 그대로 대조해 결정적으로 다시 매긴다(`원문`·`윤문`). 그러니 `quote` 는 읽은 문서(candidate)에서 한 글자도 바꾸지 말고 그대로 옮겨라. 고쳐 쓴 발췌는 대조에 걸리지 않아 유래가 틀어진다. 원본을 읽는 이유는 문제가 원문에서 왔는지 미리 가늠하기 위해서다.
