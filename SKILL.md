...
```
humanize-docs v1.2 — 대상 {N}개 파일 / run_id: {YYYY-MM-DD-NNN}
```
...
재개가 확정된 경우에는 이 배너의 `run_id`를 새로 채번하지 않고, 이어받은 run의 `run_id`를 그대로 출력한다(예: `humanize-docs v1.2 — 대상 {M}개 파일(재개) / run_id: {기존 run_id}`).
...
# (사용자 레벨 ~/.claude/skills/humanize-docs든, 프로젝트 레벨 <project>/.claude/skills/humanize-docs든,
