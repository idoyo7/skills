...
#
# ⚠ 이 함수는 반드시 `&&`/`||` 문맥에서만 불러라 — 맨 대입(`v=$(ledger_field ...)`)으로
# 부르면 큰 원장에서 **호출자가 그 자리에서 죽는다**(실측 30/30, rc=141).
# 지금 호출 지점 다섯 중 셋(cmd_run 의 cwd, cmd_link 의 cwd·session)이 이미 맨 대입이다.
#
# 근거: 아래 `sed -n ... | head -1` 에서 head -1 은 첫 줄을 읽고 즉시 파이프를 닫고,
# sed 는 일치하는 줄마다 write 한다. `^key: ` 일치 출력이 파이프 용량(이 머신 실측
# 65536바이트)을 넘으면 남은 write 가 SIGPIPE 를 맞아 141 이 된다. 그리고 이 파이프라인은
# **함수의 마지막 명령**이라 pipefail 이 올린 141 이 그대로 함수 rc → 명령 치환 rc →
# 호출자 대입의 rc 로 이어지고, 호출자 셸은 errexit 가 살아 있으므로 거기서 죽는다.
# (freeze.sh 의 detect_session() 은 같은 파이프 모양인데 마지막 명령이 basename 이라
# 141 이 함수 rc 로 새지 않는다 — 그게 그쪽이 지금 도달 불가인 이유다. 두 주석이 서로를
# 가리키고 있으니 한쪽만 고치지 마라.)
#
# 실측(이 머신, bash 3.2.57): '^cwd: ' 일치 900줄 = sed 출력 65700바이트 원장에서
#   v=$(ledger_field ...)          → 30/30 rc=141 사망
#   v=$(ledger_field ...) || v=""  → 30/30 생존   (`||` 문맥은 함수 본문 안까지 errexit 억제)
#
# 지금 잠재적인 데 그치는 이유는 페이로드뿐이다 — 실제 원장 헤더는 각 키가 한 줄씩이라
# 일치가 1~2줄이고 임계에 한참 못 미친다. 즉 안전을 지켜주는 건 호출 형태가 아니라 데이터
# 모양이다. 원장 형식이 바뀌어 같은 키가 여러 줄 나오게 되거나(예: 반복 가능한 키), 이
# 함수를 본문 절 같은 큰 구간에 쓰게 되는 날 바로 도달한다.
ledger_field() {
  local path="$1" key="$2"
